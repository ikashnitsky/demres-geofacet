...
In data visualization, it is often very challenging to represent multiple relevant dimensions preserving readability of a plot. This is especially true, when the task is to expose spatial variation of some complex phenomena. In such a case, geographical maps are the natural choice of a visualization framework -- they are meant to show spatial patterns. Yet, the usual difficulty is that one can only represent one variable meaningfully with a choropleth utilizing colors. And what if the dataset at hand is much more complex and demands a balanced exposure of several dimensions?

Usually, the difficult to represent but very important for the story telling dimension of the data is time. Visualizing time series with choropleths is very challenging. One has to produce either small multiples for the years or animated pictures with maps for various years flashing sequentially. Both variants makes it difficult to compare regions across years, which is, in fact, the main goal of such a visualization. But what if one also needs to represent in addition yet another variable? In demographic studies, such super important variable is often age. In such a case, the basic choropleth visualization framework fails. And here comes _geofaceting_.  

The idea of geofaceting is brilliantly simple: a "normal" plot is produced for each of the regions, and then all the small panels are arranged according to their approximate geographic location thereby making it easier to identify regions. The spatial logic of small-multiples alignment helps to identify the units of analysis, usually regions of a country, faster and reveals the macro-level spatial pattern while keeping the flexibility of visualization technique choice for the small-multiples themselves. As a result, creating geofaceted visualizations gives all the advantages of standard plots in which one can easily display at least three dimensions of the dataset. 

It seems that geofaceting has been invented and re-invented multiple times. Examples of a closely related idea to overlay small plots over geographical map can be seen in the works of 19-th century geographers [@palsky1996]. Geofaceting goes one step closer dropping the actual geographical map and just ordering the small-multiples in line with the spatial pattern of the corresponding areas. The approach was only recently formalized by Ryan Hafen, received its name and was consistently implemented in an R package `geofacet` [@hafen2017]. To put it in author's words:
...
To enrich the plot with geofaceted Lexis surfaces (Figure \@ref(fig:nine)), we use ternary colorcoding of the three main groups of causes of death: homicides, road traffic and suicides, and all other causes combined (Figure \@ref(fig:tern)). 
...
Basically, this plot highlights similar points as Figure \@ref(fig:nine), only it provides an opportunity to appreciate the relative importance of the two main causes of death compared with all others pulled together. For example, if we compare Federal District (the one, where the city of Mexico is located) with the neighboring Guerrero, their mortality pattern at ages 20-30 seem very similar if we look at Figure \@ref(fig:nine) and focus only at the leading cause of death, homicide. Yet, when we consider the relative importance of homicide in the mortality regime of the two states (Figure \@ref(fig:tern)), it becomes clear that homicide is by far a much bigger problem in the state of Guerrero. 
